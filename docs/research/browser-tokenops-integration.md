...
## Productized recipient portal implementation

Date: 2026-07-05. `/recipient/demo` (`components/RecipientPortal.tsx`) is no longer a walkthrough of proven spike facts — it is now the real, user-facing recipient product page, calling the proven recipient service functions from `lib/tokenops/recipient.ts` (`checkRecipientEligibility`, `grantDecryptAccess`, `decryptAllocationHandle`, `claimAllocation`, `verifyPostClaimBalance`) directly behind real, manually triggered buttons — the exact sequence the hidden diagnostic proved live in the previous checkpoint. This checkpoint is implementation + wiring only: **no button on the new page has been clicked yet** (verification was build/lint/hardhat-test/tsc + dev-server HTTP/content checks, per the same discipline as every prior phase).

### The flow (nine steps, in page order)

1. Connect wallet (existing `WalletStatusBar`/`WalletButton`) → 2. Switch to Sepolia (existing `NetworkGuard`) → 3. Import claim package → 4. Validate package → 5. Check eligibility (free) → 6. Grant decrypt access (1 paid tx, does not consume the claim) → 7. Decrypt allocation (free, optional preview) → 8. Claim allocation (1 paid tx, single-use, irreversible, heavy distinct styling) → 9. Verify post-claim confidential balance (free). A live nine-step progress timeline at the top of the page reflects real state. After claim + verify both succeed, a "Claim complete" proof panel shows the decrypted amount (or an honest "not previewed" if the optional decrypt was skipped), claim tx, post-claim balance, token/clone addresses, and network — all with Etherscan links.

### Package import model

- **Paste (textarea) or upload (plain `<input type="file">` + `FileReader` — no new dependency).** In-page instruction, verbatim: *"Your sender privately shares this claim package with you. It is not stored in the public registry."*
- **Validation** lives in a new shared pure module, `lib/distribution-import.ts`: valid JSON; `network === "Sepolia"`; `chainId === 11155111`; `tokenOpsAirdrop`/`token` valid addresses; non-empty `recipients[]`; per-recipient `wallet` address, 0x-hex `claimAuthorization`, and a required, shape-checked `encryptedInput: { handle (bytes32), inputProof (hex) }`; plus the diagnostic's real computed cross-check of the encrypted input against the package's own `encryptedHandleSummary`. Errors are collected and reported **per field** (e.g. `recipients[0].encryptedInput.handle — must be 0x-hex bytes32`), not as a generic "invalid package". The connected wallet is matched case-insensitively against `recipients[].wallet`; no match → "Connected wallet is not the recipient for this package." and every action stays disabled.
- **`recipients[].encryptedInput` is required here — no pre-fix fallback.** `lib/distribution.ts` made the field required on 2026-07-05 and every package the current `/create` flow exports carries it automatically, so the productized portal deliberately does not implement the diagnostic's manual-paste fallback for pre-fix packages; those get a specific error directing the recipient to ask their sender for a freshly exported package. (The hidden diagnostic keeps its fallback for developer use.)
- **Extraction note:** the brief-level intent was one validation module imported by both pages, but the diagnostic page is frozen as the proven developer reference (rule: do not modify `/dev/recipient-claim-diagnostic`), so it keeps its own inline copy; `lib/distribution-import.ts` is a faithful port of the same proven rules and is the forward-looking home for them, currently imported by the portal only.

### Hidden by default vs. "Advanced details"

The normal UI path shows only plain status lines — "Package loaded", "Recipient matched", "Claim authorization present", "Encrypted input present", "Allocation private until you decrypt it" — never raw signatures, handles, or proofs. An optional collapsed `<details>` disclosure ("Advanced details", off by default) shows truncated values only: clone/token addresses as Etherscan links, `shortHex`-truncated claim authorization and encrypted handle, proof **byte count** (raw bytes never rendered), the summary cross-check result, and public package metadata. The decrypted allocation itself IS shown prominently on success — it is the recipient's own number, in their own browser, which is the entire point of the product.

### Safety model (mirrors the diagnostic's proven gating)

Required checkbox, verbatim: *"I understand this claim can only be used once."* All five actions require wallet-on-Sepolia + valid package + recipient match + resolved claim material + the checkbox — enforced as real `disabled` attributes **and** in-handler refusals. Sequential gating: grant requires a successful eligibility check (paid tx, never sent blind); decrypt requires grant (hard data dependency on the granted handle); **claim requires eligibility but deliberately not decrypt** (a relayer outage on the optional preview must not block a valid claim), and additionally refuses when the latest eligibility check reported the signature invalid or preflight blocked, or when the connected wallet changed since that check ran (a hardening the portal adds over the diagnostic); verify requires claim. Errors surface through the SDK's typed codes (`isTokenOpsSdkError`, same pattern as `ExecuteStep`) translated to plain recipient-facing language in `lib/distribution-import.ts`'s `describeRecipientActionError` — e.g. `TOKENOPS_ALREADY_CLAIMED` → "This allocation has already been claimed…".

### Privacy handling (same discipline as the diagnostic)

The imported package (real wallet, plaintext amount, single-use EIP-712 claim authorization) lives **only in React component state** — never localStorage, never sessionStorage (considered and rejected: connecting an injected wallet does not navigate away, so there is no redirect to survive and no reason to widen the persistence surface), never any server, never the registry (the portal calls no registry write function at all), and it disappears on refresh. The page contains **zero `console.*` calls.** An in-page privacy panel states the registry story: the registry stores only public metadata (title, use case, token/clone addresses, recipient count); the claim package is shared privately out-of-band; recipient lists and amounts are never stored in `VantaDropRegistry`; each recipient decrypts only their own allocation. **No package JSON, claim signature, encrypted input, or plaintext amount is committed anywhere in this change** — no fixtures, no examples with real-looking claim material; the only package-shaped text anywhere is a truncated placeholder string in the textarea (`{"distributionId":"…",…}` with ellipses, no data).

### Related edits in this checkpoint

- `components/IntegrationStatus.tsx` (/verification): "Recipient decrypt/claim diagnostic: Proven live" (unchanged fact, from the 2026-07-05 live run above) and a new "Public recipient portal (/recipient/demo): **Wired — ready for live testing**" line — deliberately not "proven live", per this document's consistent wired ≠ proven distinction.
- `components/wizard/ExecuteStep.tsx` — **narrow copy-only touch, no execution-logic change**: the "Copy recipient portal instructions" text previously told recipients the in-browser claim flow "is not live yet"; it now gives the real import flow (get the claim package privately from the sender → open `/recipient/demo` with the recipient wallet on Sepolia → import → follow the steps), and the post-success "Next steps" note points at the portal instead of "the next phase".
- `app/dev/recipient-claim-diagnostic/page.tsx` is untouched and remains hidden (not linked from any nav).
